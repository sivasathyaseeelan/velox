...
class TradingSystem {
    ...
    private prepareLLMInput(data: AgentData): string {
        ...
        // Iterate through tokens and prepare the token-specific details
        for (const [token, sentiment] of Object.entries(data.tokenSentiments)) {
            ...
            input += `\n${token}:\n  - Current Market News Sentiment: ${(sentiment * 100)}% positive\n  - Price: $${price}\n  - Volume: ${volume}\n  - Market Cap: $${marketCap}\n  - Predicted Volume: ${marketPredictionVol}\n  - Predicted Price: $${marketPredictionPrice}\n  - Predicted Market Cap: $${marketPredictionCap}`;
        }
    
        input += `\n\nBased on these conditions, which tokens should we trade and in what direction? Return the response in the following format:
                        #OUTPUT FORMAT
                        [
                            {"token_name" : "token", "decision" : "buy/sell/hold" , "reason" : "logic behind your decision"}
                            ......
                        ]
    ...
    private async getLLMDecision(data: AgentData): Promise<string> {
        ...
        return response.content.toString().split("</think>")[-1];
    }
    ...
    async execute() {
        ...
        try {
            ...
            for (const token in this.tokens) {
                ...
                const tradeAmount = "10000000000000000";

                try {
                    if (signal === 'positive' && decision.toLowerCase().includes('buy')) {
                        await this.tokenManager.depositToken(token, tradeAmount);
                        console.log(`Bought token ${token} based on positive sentiment`);
                    } else if (signal === 'negative' && decision.toLowerCase().includes('sell')) {
                        await this.tokenManager.withdrawToken(token, tradeAmount);
                        console.log(`Sold token ${token} based on negative sentiment`);
                    } else {
                        console.log(`No action taken for token ${token} - Signal: ${signal}`);
                    }
                } catch (error) {
                    console.error(`Error executing trade for token ${token}:`, error);
                }
            }
        } catch (error) {
